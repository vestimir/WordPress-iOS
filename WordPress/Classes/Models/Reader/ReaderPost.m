...
- (NSString *)parseImageSrcFromHTML:(NSString *)html {
	...
	NSRange rng = [regex rangeOfFirstMatchInString:html options:0 range:NSMakeRange(0, [html length])];
...
- (BOOL)containsVideoPress:(NSString *)str {
    ...
    NSRange match = [regex rangeOfFirstMatchInString:str options:0 range:NSMakeRange(0, [str length])];
...
- (NSString *)formatVideoPress:(NSString *)str {
    ...
    NSError *error;
    NSRegularExpression *regex = [NSRegularExpression regularExpressionWithPattern:@"<div[\\S\\s]+?<div.*class=\"videopress-placeholder[\\s\\S]*?</noscript>" options:NSRegularExpressionCaseInsensitive error:&error];
    NSArray *matches = [regex matchesInString:mstr options:0 range:NSMakeRange(0, [mstr length])];
    for (NSTextCheckingResult *match in [matches reverseObjectEnumerator]) {
        ...
        NSRange mmatch = [mregex rangeOfFirstMatchInString:mstr options:0 range:match.range];
        ...
        NSRange smatch = [sregex rangeOfFirstMatchInString:mp4 options:0 range:NSMakeRange(0, [mp4 length])];
								 ...
								 success:^(AFHTTPRequestOperation *operation, id responseObject) {
									 ...
									 if (postsArr) {									 
										 [ReaderPost syncPostsFromEndpoint:path
																 withArray:postsArr
															   withContext:[[WordPressDataModel sharedDataModel] managedObjectContext]
																   success:^{
																	   if (success) {
										 ...
										 if (!loadingMore) {
											 ...
											 [ReaderPost deletePostsSyncedEarlierThan:[NSDate dateWithTimeInterval:interval sinceDate:[NSDate date]] withContext:[[WordPressDataModel sharedDataModel] managedObjectContext]];
										 }
										 return;
									 }
