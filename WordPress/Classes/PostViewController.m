...
- (void)viewDidLoad {
    ...
    UIColor *buttonTintColor = [UIColor colorFromHex:0x464646];
...
- (void)addPostObserver {
    [self.apost addObserver:self forKeyPath:KVO_POST_CONTENT options:NSKeyValueObservingOptionNew context:NULL];
}

- (void)removePostObserver {
    [self.apost removeObserver:self forKeyPath:KVO_POST_CONTENT];
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    if ([keyPath isEqualToString:KVO_POST_CONTENT]) {
        [self refreshUI];
    }
}
