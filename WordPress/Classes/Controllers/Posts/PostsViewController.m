...
- (void)viewDidLoad {
    ...
    if ([composeButtonItem respondsToSelector:@selector(setTintColor:)]) {
        composeButtonItem.tintColor = [UIColor colorFromHex:0x333333];
    }
    if (!IS_IPAD) {
...
- (void)viewWillAppear:(BOOL)animated {
	...
	if (IS_IPAD == NO) {
		...
		if (self.selectedIndexPath) {
			...
			//There is no content yet, push an the WP logo on the right.  
			[self.panelNavigationController setDetailViewController:nil];
		}
