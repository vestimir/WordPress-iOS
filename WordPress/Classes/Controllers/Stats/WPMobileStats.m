...
@property (nonatomic, strong) NSMutableDictionary *aggregatedEventProperties;
...
+ (void)pingWPComStatsEndpoint:(NSString *)statName
{
    ...
    [request setValue:[UserAgent appUserAgent] forHTTPHeaderField:@"User-Agent"];
    @autoreleasepool {
...
+ (void)resetStatsRelatedVariables
{
    [WPMobileStats clearPropertiesForAllEvents];
    [WPMobileStats sharedInstance].hasRecordedApplicationOpenedEvent = false;
}

+ (void)recordAppLaunched {
    [WPMobileStats sharedInstance].hasRecordedApplicationOpenedEvent = true;
}

+ (BOOL)hasRecordedAppLaunched {
    return [WPMobileStats sharedInstance].hasRecordedApplicationOpenedEvent;
}
