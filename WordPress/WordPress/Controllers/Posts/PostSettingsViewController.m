...
- (void)datePickerChanged {
	...
	[_postDetailViewController refreshButtons];
    [tableView reloadData];
}
    ...
    case 1: // Post format
        {
            ...
            return _postFormatTableViewCell;
        }
	case 2: 
        if (blogSupportsFeaturedImage) {
            if (!self.post.post_thumbnail && !isUploadingFeaturedImage) {
            ...
            if (blogSupportsFeaturedImage) {
                ...
                switch (indexPath.row) {
                    ...
                    case 1:
                        ...
                        [actionSheet showFromRect:cell.frame inView:_postDetailViewController.view animated:YES];
                        break;
                }
            } else {
...
- (void)geolocationCellTapped:(NSIndexPath *)indexPath {
    switch (indexPath.row) {
        ...
        case 2:
            ...
            _postDetailViewController.hasLocation.enabled = NO;
            [_postDetailViewController refreshButtons];
            break;
    }
    [tableView reloadData];
    ...
    if (IS_IPAD) {
        ...
        if (picker.tag == TAG_PICKER_DATE) {
            ...
            if ([publishNowButton respondsToSelector:@selector(setTintColor:)]) {
                publishNowButton.tintColor = _postDetailViewController.toolbar.tintColor;
            }
        ...
        CGFloat width = _postDetailViewController.view.frame.size.width;
        CGFloat height = 0.0;
        ...
        if(_postDetailViewController.view.frame.size.height <= 416.0f) {
            height = 490.0f;
        } else {
            height = 500.0f;
        }
        ...
        [actionSheet showInView:_postDetailViewController.view];
...
- (void)locationManager:(CLLocationManager *)manager
    ...
    if (abs(howRecent) < 15.0)
    {
		...
		_postDetailViewController.hasLocation.enabled = YES;
