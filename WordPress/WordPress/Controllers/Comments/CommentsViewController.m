...
@property IBOutlet UIBarButtonItem *approveButton;
@property IBOutlet UIBarButtonItem *unapproveButton;
@property IBOutlet UIBarButtonItem *spamButton;
...
- (void)viewDidLoad {
    ...
    _spamButton = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"toolbar_flag"] style:UIBarButtonItemStylePlain target:self action:@selector(spamSelectedComments:)];
    _unapproveButton = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"toolbar_unapprove"] style:UIBarButtonItemStylePlain target:self action:@selector(unapproveSelectedComments:)];
    _approveButton = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"toolbar_approve"] style:UIBarButtonItemStylePlain target:self action:@selector(approveSelectedComments:)];
    _deleteButton = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"toolbar_delete"] style:UIBarButtonItemStylePlain target:self action:@selector(confirmDeletingOfComments)];

    if (IS_IPHONE) {
        UIBarButtonItem *spacer = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemFlexibleSpace target:nil action:nil];
        self.toolbarItems = [NSArray arrayWithObjects:_approveButton, spacer, _unapproveButton, spacer, _spamButton, spacer, _deleteButton, nil];
    }
...
- (void)setEditing:(BOOL)editing animated:(BOOL)animated {
    ...
    if (IS_IPHONE) {
        ...
        if (editing) { 
            ...
            self.toolbarItems = [NSArray arrayWithObjects:self.editButtonItem, spacer,  _approveButton, spacer, spacer, _unapproveButton, spacer, spacer, _spamButton, spacer, spacer, _deleteButton, spacer, nil];
        } else {
            self.toolbarItems = [NSArray arrayWithObject:self.editButtonItem];
        }
    ...
    [_approveButton setEnabled:!editing];
    [_unapproveButton setEnabled:!editing];
    [_spamButton setEnabled:!editing];
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
    ...
    if(buttonIndex == 0) {
        [self deleteSelectedComments:_deleteButton];
    }
}
...
- (void)updateSelectedComments {
    ...
    [_approveButton setEnabled:((count - approvedCount) > 0)];
    [_unapproveButton setEnabled:((count - unapprovedCount) > 0)];
    [_spamButton setEnabled:((count - spamCount) > 0)];
