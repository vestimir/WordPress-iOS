...
static NSString *const KVO_COMMENT_STATUS = @"status";
static NSString *const KVO_BLOG_REACHABLE = @"reachable";
...
- (void)dealloc {
    ...
    [self.comment removeObserver:self forKeyPath:KVO_COMMENT_STATUS];
    [_comment.blog removeObserver:self forKeyPath:KVO_BLOG_REACHABLE];
...
- (void)insertPendingLabel {
    ...
    pendingLabelHolder.backgroundColor = [UIColor colorFromHex:0xf6f6dc alpha:1.0f];
...
- (void)setComment:(Comment *)comment {
    ...
    [_comment.blog removeObserver:self forKeyPath:KVO_BLOG_REACHABLE];
    [_comment removeObserver:self forKeyPath:KVO_COMMENT_STATUS];
    
    [self willChangeValueForKey:@"comment"];
    _comment = comment;
    [self didChangeValueForKey:@"comment"];
    
    [_comment addObserver:self forKeyPath:KVO_COMMENT_STATUS options:0 context:NULL];
    [_comment.blog addObserver:self forKeyPath:KVO_BLOG_REACHABLE options:NSKeyValueObservingOptionNew context:NULL];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    ...
    if ([keyPath isEqualToString:KVO_COMMENT_STATUS]) {
        [self showComment:self.comment];
    } else if ([keyPath isEqualToString:@"reachable"]) {
